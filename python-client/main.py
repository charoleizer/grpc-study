...
import protos.example_pb2 as pb
...
stub = protos.example_pb2_grpc.TransformStub(channel)
...
def paint(event):
    ...
    for flip_point in stub.flip(send([event_point])):
        sys.stdout.write('\rSent: ({0},{1})  Received: ({2},{3})'.format(
            event_point.x, event_point.y, flip_point.x, flip_point.y))
        sys.stdout.flush()
